...
from models.zhipuai.exception_classifier import classify, safe_log_payload
...
class ZHIPUAIBot(Bot, ZhipuAIImage):
    ...
    def __init__(self):
        ...
        self.args = {
            "model": conf().get("model") or "glm-4",
            "temperature": conf().get("temperature", 0.9),
            "top_p": conf().get("top_p", 0.7),
        }
        # Per-instance client holder — main chat and Scheduler never share
        api_key = conf().get("zhipu_ai_api_key")
        api_base = conf().get("zhipu_ai_api_base")
        self._client_holder = ClientHolder(api_key, api_base)
        self.client = self._client_holder.get_or_create()

    # ── Client lifecycle ──────────────────────────────
    def capture_client_gen(self) -> int:
        """Record the generation at request-start. Call before each API request."""
        return self._client_holder.get_generation()

    def _reset_client(self, expected_generation: int, reason: str = "") -> bool:
        """CAS-based rebuild: only rebuilds if generation == expected_generation.

        Caller must pass the generation captured BEFORE the failed request.
        This method does NOT re-read the current generation internally.
        """
        ok = self._client_holder.rebuild(expected_generation, reason)
        # Always refresh self.client to the current (possibly new) client
        self.client = self._client_holder.get_or_create()
        return ok

    def _get_client_gen(self) -> int:
        return self._client_holder.get_generation()
    ...
    def call_with_tools(self, messages, tools=None, stream=False, **kwargs):
        ...
        try:
            ...
            # ── P0.2: Raise ProviderAPIError to preserve structured fields ──
            info = classify(e)
            model_name = request_params.get("model", "unknown")
            logger.error(
                "[ZHIPU_AI] call_with_tools error: %s",
                safe_log_payload(info, model_name, 0, self._get_client_gen())
            )
            raise ProviderAPIError.from_classified(str(e)[:200], info)
    ...
    def _handle_sync_response(self, request_params):
        ...
        try:
            ...
            info = classify(e)
            logger.error("[ZHIPU_AI] sync error: %s", safe_log_payload(
                info, str(request_params.get("model", "?")), 0, self._get_client_gen()))
            raise ProviderAPIError.from_classified(str(e)[:200], info)
    ...
    def _handle_stream_response(self, request_params):
        ...
        try:
            ...
            info = classify(e)
            logger.error("[ZHIPU_AI] stream error: %s", safe_log_payload(
                info, str(request_params.get("model", "?")), 0, self._get_client_gen()))
            yield {
                "error": True,
                "message": str(e)[:200],
                ...
                "is_connection_error": info["is_connection_error"],
                "is_timeout": info["is_timeout"],
                "is_rate_limit": info["is_rate_limit"],
                "is_server_error": info["is_server_error"],
                "is_client_error": info["is_client_error"],
                "request_id": info.get("request_id"),
                "_provider_error_fields": info,
            }
