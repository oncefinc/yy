...
def _configure_initiative_delivery(channel_mgr):
    """Bind Initiative to the already-created production Weixin channel."""
    try:
        from cow.initiative_engine.config import DELIVERY_ENABLED
        from cow.initiative_engine.delivery import configure_delivery
        if not DELIVERY_ENABLED:
            configure_delivery(None)
            return
        channel = (channel_mgr.get_channel(const.WEIXIN)
                   or channel_mgr.get_channel("wx"))
        if channel is None or not hasattr(channel, "send_proactive_text"):
            configure_delivery(None)
            logger.warning("[App] Initiative delivery unavailable: Weixin channel missing")
            return

        prefix = str(conf().get("single_chat_reply_prefix", "") or "")

        def _deliver(receiver: str, message: str) -> bool:
            payload = message
...
def run():
    ...
    try:
        ...
        _configure_initiative_delivery(_channel_mgr)
