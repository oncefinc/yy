...
class WeixinChannel(ChatChannel):
    ...
    def _process_message(self, raw_msg: dict):
        ...
        if wx_msg.ctype == ContextType.IMAGE:
            if hasattr(wx_msg, "image_path") and wx_msg.image_path:
                # Vision Bridge v1: validate + encode image for GLM-5V
                try:
                    from bridge.vision_bridge import process_image
                    img_data = process_image(wx_msg.image_path)
                    if img_data:
                        file_cache.add(session_id, img_data, file_type="image_base64")
                        logger.info(f"[Weixin] Image encoded for Vision: {img_data['size_bytes']}B {img_data['mime']}")
                    else:
                        # Fallback: store path as text reference
                        file_cache.add(session_id, wx_msg.image_path, file_type="image")
                        logger.info(f"[Weixin] Image cached (text fallback) for session {session_id}")
                except Exception as e:
                    logger.warning(f"[Weixin] Vision Bridge failed, fallback to text: {e}")
                    file_cache.add(session_id, wx_msg.image_path, file_type="image")
            return
        ...
        if wx_msg.ctype == ContextType.TEXT:
            ...
            if cached_files:
                # Vision Bridge: build multimodal content blocks
                vision_images = [fi for fi in cached_files if fi["type"] == "image_base64"]
                other_files = [fi for fi in cached_files if fi["type"] != "image_base64"]

                if vision_images:
                    from bridge.vision_bridge import build_multimodal_content
                    image_dicts = [fi["path"] for fi in vision_images]  # path field holds the encoded dict
                    multimodal = build_multimodal_content(image_dicts, wx_msg.content)
                    # Store multimodal blocks in the message for _compose_context
                    wx_msg._multimodal_content = multimodal
                    logger.info(f"[Weixin] Vision: {len(vision_images)} image(s) merged with text")
        ...
        context_kwargs = {}
        if hasattr(wx_msg, '_multimodal_content'):
            context_kwargs['multimodal_content'] = wx_msg._multimodal_content
    ...
    def send_proactive_text(self, receiver: str, text: str) -> bool:
        """Send one short proactive message and report the API outcome.

        Initiative messages use the persisted per-user context token collected
        by the normal receive path.  The method deliberately returns a boolean
        so a failed API call is never recorded as a successful delivery.
        """
        context_token = self._get_context_token(receiver)
        text = (text or "").strip()
        if not receiver or not text or not context_token:
            logger.warning("[Weixin] Proactive send skipped: missing receiver/text/token")
            return False
        try:
            ...
            if isinstance(resp, dict):
                ...
                if ret not in (None, 0):
                    logger.warning("[Weixin] Proactive send rejected: code=%s", ret)
                    return False
            logger.info("[Weixin] Proactive text sent, len=%d", len(text))
            return True
        except Exception as exc:
            logger.error("[Weixin] Proactive send failed: %s", type(exc).__name__)
            return False
