...
class ChatChannel(Channel):
    ...
    def _handle(self, context: Context):
        if context is None or not context.content:
            return
        # ── Intercept image download failure before it reaches AgentBridge ──
        cmsg = context.get("msg")
        if cmsg and getattr(cmsg, '_download_failed', False):
            logger.warning("[chat_channel] image download failed, sending direct reply")
            reply = Reply(ReplyType.TEXT, "图片下载失败了，请稍后重新发送一次。")
