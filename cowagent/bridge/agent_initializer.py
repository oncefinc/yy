...
class AgentInitializer:
    ...
    def initialize_agent(self, session_id: Optional[str] = None) -> Agent:
        ...
        # Temporal + Reality Grounding rules (domain-agnostic)
        agent.extra_system_suffix = (
            "【时间规则】\n"
            "- 每条用户消息前的 [时间戳｜相对时间] 标注了该消息的发生时刻。\n"
            "- \"正在、还在、刚刚、马上\"等状态词只描述消息发生当时，跨越明显时间间隔后不得默认该状态仍持续。\n"
            "- 引用过去消息中的状态时必须使用\"昨晚、之前、当时\"等过去时间表达。\n\n"
            "【Reality Grounding 规则】\n"
            "- 记忆召回后的标签标注了证据类型：[当前明确事实]/[习惯参考]/[历史事件]/[时间不确定]/[图片观察]/[推断]。\n"
            "- 当前状态默认 unknown。计划≠正在进行。正在进行≠已完成。历史事件≠当前状态。习惯≠今天发生。\n"
            "- 只有标注为 [当前明确事实] 或 [近期明确事实] 的记忆可以作为当前状态使用。\n"
            "- 标注为 [习惯/参考｜不代表当前发生] 的内容只代表一般规律，不要当作今天已经发生的事情。\n"
            "- 图片观察只能描述画面内容，不能直接推断用户当前所在地或正在做的活动。\n"
            "- 不确定当前状态时使用条件句（\"如果今天...\"）或直接询问，不要自行补全。\n"
            "- 用户纠正事实时，新的明确证据覆盖旧推断，但不能连带修正其他无关事实。"
        )
        # AgentBridge appends a live capability/action snapshot before every
        # turn.  Keep the static grounding rules separately so refreshing the
        # runtime suffix never duplicates or loses them.
        agent._grounding_system_suffix = agent.extra_system_suffix
    ...
    def _filter_text_only_messages(messages: list) -> list:
        ...
        # Build result from completed pairs only.  A trailing user message can
        # be left behind when the process is restarted while a reply is still
        # running.  Restoring that dangling question next to the new live query
        # makes the model answer the old question first (cross-topic reply).
        # The current live message is appended separately by AgentBridge, so a
        # dangling persisted user turn is never valid history here.
        filtered = []
        ...
        for turn in turns:
            ...
            if not user_text:
                continue
            # Inject time annotation from created_at if available
            created_at = turn["user"].get("created_at")
            time_prefix = ""
            if created_at:
                try:
                    from datetime import datetime
                    dt = datetime.fromtimestamp(created_at, tz=CST)
                    gap_min = (now_ts - created_at) // 60
                    if gap_min < 1:
                        rel = "刚刚"
                    elif gap_min < 60:
                        rel = f"{gap_min}分钟前"
                        ...
                        h = gap_min // 60
                        # Detect yesterday vs same-day
                        now_dt = datetime.fromtimestamp(now_ts, tz=CST)
                        if dt.date() == now_dt.date():
                            rel = f"{h}小时前"
                        else:
                            rel = f"昨晚" if 18 <= dt.hour or dt.hour < 6 else f"昨天{dt.strftime('%H:%M')}"
                    else:
                        days = gap_min // 1440
                        rel = f"{days}天前" if days <= 7 else dt.strftime("%m-%d")
                    time_prefix = f"[{dt.strftime('%Y-%m-%d %H:%M')} Asia/Shanghai｜{rel}]\n"
                except:
                    pass
            filtered.append({
                "role": "user",
                "content": [{"type": "text", "text": f"{time_prefix}{user_text}"}]
            })
