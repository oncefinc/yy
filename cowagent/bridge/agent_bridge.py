...
def _ensure_cow_extension_path() -> None:
...
class AgentBridge:
    ...
    def __init__(self, bridge: Bridge):
        ...
        # Start Initiative Engine runtime (Phase 2: shadow-only, daemon thread).
        try:
            _ensure_cow_extension_path()
            # Configure LLM adapter (inject CowAgent's Zhipu config)
            from config import conf
            from cow.initiative_engine.zhipu_adapter import create_zhipu_generator
            from cow.initiative_engine.llm_worker import configure as llm_configure
            api_key = conf().get("zhipu_ai_api_key", "")
            api_base = conf().get("open_ai_api_base", "https://open.bigmodel.cn/api/paas/v4")
            if api_key:
                llm_configure(create_zhipu_generator(api_key, api_base))
                logger.info("[AgentBridge] Initiative LLM adapter configured")
            # Start runtime daemon
            from cow.initiative_engine.runtime import start_runtime
            if start_runtime():
                logger.info("[AgentBridge] Initiative runtime daemon started")
            # Deployment path audit log
            import hashlib
            for label, mod in [("memory_engine","cow.memory_engine"),
                               ("initiative_engine","cow.initiative_engine"),
                               ("context_builder","cow.initiative_engine.context_builder")]:
                try:
    ...
    @staticmethod
    def _on_agent_session_start(session_id: str = None):
        """Notify memory engine that a new agent session has started."""
        try:
            _ensure_cow_extension_path()
            from cow.memory_engine.integration import on_session_start
            on_session_start()
        except Exception as e:
            logger.info(f"[AgentBridge] Memory engine session-start skipped: {e}")
    ...
    def agent_reply(self, query: str, context: Context = None,
        ...
        steer_inbox = None
        try:
            logger.info(f"[AgentBridge] agent_reply() ENTER — query={query[:50]}...")
            ...
            # The same identifier scopes daily memory and Memory V2 writes.
            # Group/scheduler paths are excluded by their callers/guards.
            ...
            try:
                ...
                try:
                    _ensure_cow_extension_path()
                    from cow.memory_engine.integration import recall_context
                    memory_ctx = recall_context(
                        query, receiver_id=str(session_id or "")
                    )
                    if memory_ctx:
                        enhanced_query = f"{query}\n\n[{memory_ctx}]"
                        logger.info(f"[AgentBridge] 💉 Memory injected ({len(memory_ctx)} chars)")
                    else:
                        logger.info(f"[AgentBridge] 🔍 Memory recall returned empty (engine ok, no matches)")

                    # ── V2 Shadow: bge-base semantic-only (pure, no BM25/boost/cross-encoder) ──
                    try:
                        from cow.memory_engine.shadow import run_shadow_query
                        run_shadow_query(query, session_id or "", memory_ctx)
                    except Exception as se:
                        logger.debug(f"[AgentBridge] V2 shadow skipped: {se}")

                except Exception as e:
                    logger.info(f"[AgentBridge] ❌ Memory recall skipped: {e}")

                # ── Temporal Cognition T3A.1: update World State from real user messages ──
                try:
                    _ensure_cow_extension_path()
                    from cow.temporal_cognition.config import (
                        TEMPORAL_ENGINE_ENABLED, TEMPORAL_INGEST_ENABLED,
                        TEMPORAL_PROMPT_ENABLED,
                    )
                    if TEMPORAL_ENGINE_ENABLED and TEMPORAL_INGEST_ENABLED:
                        cmsg = context.get("msg") if context else None
                        is_user_msg = (
                            cmsg is not None
                            and not getattr(cmsg, 'my_msg', False)
                            and not (context.get("is_scheduled_task"))
                        )
                        if is_user_msg:
                            # ── Initiative Engine: track real user chat activity ──
                            try:
                                receiver = getattr(cmsg, 'from_user_id', '') or session_id or ''
                                # Only single-chat (non-group), real user messages
                                is_group = bool(context.get("isgroup")) if context else False
                                if receiver and not is_group:
                            ...
                            from cow.temporal_cognition.clock import now as tc_now

                            # Primary: WeChat message_id from platform
                            event_id = str(getattr(cmsg, 'msg_id', '') or '')
                            # Secondary: stable hash of (user, content, create_time)
                            if not event_id:
                                import hashlib
                                raw = (
                                    f"{getattr(cmsg, 'from_user_id', '')}:"
                                    f"{query}:"
                                    f"{getattr(cmsg, 'create_time', '')}"
                                )
                            ...
                            event = IngressEvent(
                                event_id=event_id,
                                source="wechat_text",
                                sender_id=str(getattr(cmsg, 'from_user_id', '')),
                                received_at=tc_now().isoformat(),
                                content=query,
                                metadata={
                                    "wx_create_time_ms": getattr(cmsg, 'create_time', 0),
                                    "session_id": session_id or "",
                                },
                            )
                            tc_result = process_message(event)
                            logger.info(
                                "[AgentBridge] 🕐 Temporal: %s mutations, %s facts, "
                                "%.1f ms",
                                tc_result.get("mutation_count", 0),
                                tc_result.get("current_fact_count", 0),
                                tc_result.get("latency_ms", 0),
                            )
                            temporal_context = tc_result.get("rendered_context", "")
                            if TEMPORAL_PROMPT_ENABLED and temporal_context:
                                # The ledger contains only fresh explicit facts,
                                # recent completed events and clearly-labelled
                                # stale inquiry hints.  It may constrain a reply,
                                # but must never be expanded into unstated facts.
                                enhanced_query = (
                                    f"{enhanced_query}\n\n{temporal_context}\n"
                                    "[Temporal 使用规则：fresh 可作为当前事实；"
                                    "stale 只能用问句确认；unknown 不得自行补全。]"
                                )
                                logger.info(
                                    "[AgentBridge] 🕐 Temporal context injected (%s chars)",
                                    len(temporal_context),
                                )
                except Exception as e:
                    logger.info("[AgentBridge] 🕐 Temporal cognition skipped (non-fatal): %s", e)

                # ── Dynamic self-awareness: live tools + execution receipts ──
                # This belongs in the system suffix rather than long-term
                # memory.  It is rebuilt every turn, so hot-loaded tools and
                # background actions are represented truthfully after restart.
                try:
                    from cow.self_awareness.capabilities import render_runtime_context
                    grounding = getattr(
                        agent, "_grounding_system_suffix",
                        getattr(agent, "extra_system_suffix", "") or "",
                    )
                    awareness = render_runtime_context(agent, str(session_id or ""))
                    agent.extra_system_suffix = f"{grounding}\n\n{awareness}".strip()
                    logger.info(
                        "[AgentBridge] 🧭 Runtime self-awareness refreshed (%s chars)",
                        len(awareness),
                    )
                except Exception as e:
                    logger.info(
                        "[AgentBridge] 🧭 Runtime self-awareness skipped (non-fatal): %s",
                        type(e).__name__,
                    )

                # Check for multimodal content (Vision Bridge)
                multimodal = context.get("multimodal_content") if context else None

                # Vision proxy: DeepSeek observes pixels; the primary GLM keeps
                # ownership of conversation, memory, tools and personality.
                if multimodal:
                    try:
                        from bridge.vision_proxy import proxy_images_for_primary
                        multimodal, proxy_meta = proxy_images_for_primary(
                            multimodal,
                            user_query=query,
                            enhanced_query=enhanced_query,
                        )
                        logger.info(
                            "[AgentBridge] 👁️ Vision proxy: used=%s reason=%s images=%s",
                            proxy_meta.get("used", False),
                            proxy_meta.get("reason", "unknown"),
                            proxy_meta.get("image_count", 0),
                        )
                    except Exception as e:
                        # Perception routing must never make image chat unusable.
                        logger.warning(
                            "[AgentBridge] 👁️ Vision proxy skipped (%s); primary fallback",
                            type(e).__name__,
                        )
            ...
            # ── Initiative Engine: track successful assistant reply ──
            try:
                if session_id and not session_id.startswith("scheduler_"):
                    is_group = bool(context.get("isgroup")) if context else False
                    if not is_group:
    ...
    def clear_session(self, session_id: str):
        ...
        if session_id in self.agents:
            ...
            try:
                _ensure_cow_extension_path()
                from cow.memory_engine.integration import on_session_end
                on_session_end()
            except Exception as e:
                logger.info(f"[AgentBridge] Memory engine session-end skipped: {e}")
