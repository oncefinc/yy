...
- 每条一行，必须以“- ”开头，并严格使用以下格式之一：
  - [事实] 归纳内容 | 依据：“逐字复制的用户原话”
  - [事件] 归纳内容 | 依据：“逐字复制的用户原话”
  - [计划] 归纳内容 | 依据：“逐字复制的用户原话”
  - [偏好] 归纳内容 | 依据：“逐字复制的用户原话”
  - [决定] 归纳内容 | 依据：“逐字复制的用户原话”
- 合并同一件事的多轮对话
- 只记录有意义的事件，忽略闲聊和问候
- 保留关键的决策、结论和待办事项
- 只把用户明确陈述当作事实；助手的话只能帮助理解上下文，不能成为新事实来源
- “依据”必须是对话中某一条“用户:”消息的连续原文，不得改写、拼接或引用助手的话
- 如果找不到能够直接支持归纳内容的用户原话，就不要输出该条
- 不得根据图片内容、视觉模型描述、语气或行为推测身体、情绪、位置等事实
- 用户没有亲口确认的提醒、建议和待办，不得写成用户计划
- 不记录银月、CowAgent、模型、记忆系统或代码自身的能力和架构
- 不记录助手声称自己已经搜索、保存、修改、发送或设置提醒等行为
- 发生纠正时只写纠正后的事实，不保留被否定的旧说法
- 把“今天、昨天、明天”等相对日期改写成相对于记录日期的明确日期
- 每次最多输出 5 条；宁缺毋滥，不确定就不记录
...
- One item per line, beginning with "- ", in exactly this form:
  - [Fact] summary | Evidence: "verbatim user quote"
  - [Event] summary | Evidence: "verbatim user quote"
  - [Plan] summary | Evidence: "verbatim user quote"
  - [Preference] summary | Evidence: "verbatim user quote"
  - [Decision] summary | Evidence: "verbatim user quote"
...
- Only explicit user statements may establish facts; assistant statements are context, not evidence
- Evidence must be one continuous verbatim excerpt from a `User:` turn; never paraphrase, concatenate, or quote the assistant
- Omit an item if no user quote directly supports it
- Do not infer body, emotion, location, or other facts from images, model descriptions, tone, or behavior
- Do not turn an assistant reminder or suggestion into a user plan
- Do not record facts about the assistant, CowAgent, models, memory architecture, or code
- Do not record assistant claims about having searched, saved, edited, sent, or scheduled something
- If corrected, keep only the corrected fact
- Rewrite relative dates into explicit dates based on the record date
- Output at most 5 items; omit anything uncertain

If the conversation has no record value (only greetings or meaningless content), reply with exactly "None"."""

SUMMARIZE_USER_PROMPT_ZH = """记录日期：{date}

请归纳以下对话的日常记录：

{conversation}"""

SUMMARIZE_USER_PROMPT_EN = """Record date: {date}

Summarize the daily record of the following conversation:
...
class MemoryFlushManager:
    ...
    def write_daily_summary(
        ...
        try:
            ...
            # Memory V2 is the canonical atom store.  Daily markdown remains
            # the human-readable source, while tagged, user-grounded bullets
            # are mirrored into V2 + the bge-base projection.  This is
            # deliberately fail-open: consolidation must never break chat or
            # the daily markdown write.
            if user_id and not str(user_id).startswith("scheduler_"):
                try:
                    from config import conf
                    if conf().get("memory_v2_daily_sync_enabled", False):
                        from cow.memory_engine.daily_sync import sync_daily_summary
                        result = sync_daily_summary(
                            summary,
                            receiver_id=str(user_id),
                            record_date=datetime.now().date(),
                            source_file=str(daily_file),
                            user_evidence_texts=self._extract_user_evidence_texts(
                                source_messages or []
                            ),
                        )
                        logger.info(
                            "[MemoryFlush] V2 daily sync: "
                    ...
                    logger.warning(
                        f"[MemoryFlush] V2 daily sync failed-open: {type(sync_error).__name__}"
                    )
            self.last_flush_timestamp = datetime.now()
            return True
        except Exception as e:
    ...
    def _clean_summary_output(raw: str) -> str:
        ...
        # Some providers omit the bullet even when prompted.  Normalize the
        # human-readable daily format here; the V2 parser still requires a
        # verified verbatim user-evidence suffix before it will write.
        tag_line = re.compile(
            r"^\s*\[(事实|事件|计划|偏好|决定|Fact|Event|Plan|Preference|Decision)\]",
            re.IGNORECASE,
        )
        raw = "\n".join(
            ("- " + line.lstrip()) if tag_line.match(line) else line
    ...
    def _write_dream_diary(self, content: str, user_id: Optional[str] = None):
        ...
        overwriting.  This prevents data loss from multiple dream triggers
        within the same day (e.g., manual /memory dream + scheduled run).
        """
        ...
        if diary_file.exists():
            logger.warning(
                f"[DeepDream] {today}.md already exists! "
                f"Appending new section instead of overwriting."
            )
            with open(diary_file, "a", encoding="utf-8") as f:
                f.write(
                    f"\n\n---\n\n"
                    f"## 补记 | {datetime.now().strftime('%H:%M')}\n\n"
                    f"{content}\n"
                )
            logger.info(
                f"[DeepDream] Appended dream section to existing {diary_file}"
            )
        else:
    ...
    @classmethod
    def _extract_user_evidence_texts(cls, messages: List[Dict]) -> List[str]:
        """Return only user-authored text eligible as grounding evidence."""
        evidence = []
    ...
    def _call_llm_for_summary(self, conversation_text: str) -> str:
        ...
        request = LLMRequest(
            messages=[{"role": "user", "content": _summarize_user_prompt().format(
