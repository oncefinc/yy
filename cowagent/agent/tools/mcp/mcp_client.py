...
class McpClient:
    ...
    def _init_stdio(self) -> bool:
        ...
        # Windows CreateProcess does not consistently resolve npm's extensionless
        # ``npx`` shim.  Resolve through PATHEXT and run .cmd/.bat shims via the
        # system command processor.  This keeps portable mcp.json files working
        # without hard-coding a machine-specific Node installation path.
        resolved = shutil.which(str(command), path=env.get("PATH")) or str(command)
        argv = [resolved] + list(args)
        if os.name == "nt" and resolved.lower().endswith((".cmd", ".bat")):
            comspec = env.get("COMSPEC") or os.environ.get("COMSPEC") or "cmd.exe"
            # ``call`` is important here.  Without it, cmd.exe parses a batch
            # path such as ``C:\Program Files\nodejs\npx.cmd`` as ``C:\Program``
            # even though Popen received a correctly separated argv list.
            # Keeping the path and arguments as separate items also avoids
            # building a shell command string from config values.
            argv = [comspec, "/d", "/s", "/c", "call", resolved] + list(args)
