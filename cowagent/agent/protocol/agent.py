...
class Agent:
    ...
    def run_stream(self, user_message: str, on_event=None, clear_history: bool = False,
        ...
        # Execute
        try:
            response = executor.run_stream(user_message, content_blocks=content_blocks)
        except Exception:
    ...
    def compact_context(self, keep_recent_turns: int = 2) -> dict:
        ...
        if flush_mgr and llm_summary:
            try:
                ...
                flush_mgr.write_daily_summary(
                    summary,
                    user_id=user_id,
                    reason="trim",
                    source_messages=discarded_messages,
                )
            except Exception as e:
