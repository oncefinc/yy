...
class AgentStreamExecutor:
    ...
    def __init__(
        ...
        # Successful calls in this executor only. This is the authority for
        # completed-action claims in the final response.
    ...
    def run_stream(self, user_message: str, content_blocks: list = None) -> str:
        """
        Execute streaming reasoning loop

        Args:
            user_message: User message (text, for logging)
            content_blocks: Optional multimodal content blocks for Vision

        Returns:
            Final response text
        """
        # Log user message with model info. Truncate very long messages
        ...
        img_count = sum(1 for b in (content_blocks or []) if b.get("type") == "image_url")
        img_label = f" | 🖼️ x{img_count}" if img_count else ""
        logger.info(f"🤖 {self.model.model}{thinking_label}{img_label} | 👤 {_log_msg}")
        ...
        try:
            ...
            # Completed-action language must be backed by a successful tool in
            # this run (or by this turn's visual evidence). Fail open if the
            # guard itself encounters an implementation error.
            try:
                from config import conf
                gate_enabled = bool(conf().get("action_truth_gate_enabled", True))
                if gate_enabled and final_response and not cancelled:
                    from cow.self_awareness.action_truth import enforce_action_truth
                    vision_grounded = any(
                        isinstance(block, dict)
                        and block.get("type") == "text"
                        and "[图像观察｜来自独立视觉模型" in str(block.get("text") or "")
                        for block in (content_blocks or [])
                    )
                    truth = enforce_action_truth(
                        final_response,
                        successful_tools=self.successful_tool_calls,
                        vision_grounded=vision_grounded,
                    )
                    if truth.changed:
                        original_response = final_response
                        final_response = truth.text
                        # Keep persisted history identical to the text returned
                        # to WeChat by replacing the final assistant text block.
                        for message in reversed(self.messages):
                            ...
                            for block in reversed(blocks):
                                if (isinstance(block, dict)
                                        and block.get("type") == "text"
                                        and block.get("text") == original_response):
                                    block["text"] = final_response
                                    break
                            break
                        logger.warning(
                            "[ActionTruth] repaired unsupported action claim(s): %s",
                            ",".join(truth.blocked_action_types),
                        )
            except Exception as truth_error:
                logger.warning(
                    "[ActionTruth] gate skipped (non-fatal): %s",
                    type(truth_error).__name__,
                )
            if cancelled:
    ...
    def _call_llm_stream(self, retry_on_empty=True, retry_count=0, max_retries=3,
        ...
        # ── Anonymous payload diagnostic (fail-open — never blocks chat) ──
        try:
            payload_diag = self._diagnose_payload(messages, tools_schema)
            logger.info(
                "📊 Payload: %d msgs (%d turns) | images=%d(%d chars) "
                "| current_img=%d(%d) hist_img=%d(%d) "
                "| tool_use=%d tool_result=%d pair_ok=%s | empty=%d | "
                "max_chunk=%d chars | total=%d chars | tools=%d | model=%s",
                payload_diag.get("message_count", 0),
                payload_diag.get("turn_count", 0),
                payload_diag.get("image_count", 0),
                payload_diag.get("image_chars", 0),
                payload_diag.get("current_image_count", 0),
                payload_diag.get("current_image_chars", 0),
                payload_diag.get("historical_image_count", 0),
                payload_diag.get("historical_image_chars", 0),
                payload_diag.get("tool_use_count", 0),
                payload_diag.get("tool_result_count", 0),
                payload_diag.get("tool_pair_ok", True),
                payload_diag.get("empty_content_count", 0),
                payload_diag.get("max_content_chars", 0),
                payload_diag.get("payload_total_chars", 0),
                payload_diag.get("tools_count", 0),
                payload_diag.get("model", "?"),
            )
        except Exception:
            logger.warning("📊 Payload diagnostic failed — skipping (non-fatal)")

        # ── Capture client generation BEFORE the request (for CAS on error) ──
        request_client_gen = -1
        bot = getattr(self.model, 'bot', None)
        if bot and hasattr(bot, 'capture_client_gen'):
            request_client_gen = bot.capture_client_gen()
        ...
        try:
            ...
            for chunk in stream:
                ...
                if isinstance(chunk, dict) and chunk.get("error"):
                    ...
                    # ── P0.2: Build structured error from chunk fields ──
                    provider_fields = chunk.get("_provider_error_fields", {})
                    is_connection = chunk.get("is_connection_error", False)
                    is_timeout = chunk.get("is_timeout", False)
                    is_rate_limit = chunk.get("is_rate_limit", False)
                    is_server_error = chunk.get("is_server_error", False)

                    # Context overflow detection (keyword-based fallback)
                    ...
                    # Build ProviderAPIError with structured fields
                    from models.zhipuai.provider_api_error import ProviderAPIError
                    raise ProviderAPIError(
                        message=error_msg[:200],
                        status_code=status_code if isinstance(status_code, int) else None,
                        error_type=chunk.get("error_type", provider_fields.get("error_type", "")),
                        retryable=chunk.get("retryable", True),
                        is_connection_error=is_connection or provider_fields.get("is_connection_error", False),
                        is_timeout=is_timeout or provider_fields.get("is_timeout", False),
                        is_rate_limit=is_rate_limit or provider_fields.get("is_rate_limit", False),
                        is_server_error=is_server_error or provider_fields.get("is_server_error", False),
                        is_client_error=chunk.get("is_client_error", provider_fields.get("is_client_error", False)),
                        request_id=chunk.get("request_id") or provider_fields.get("request_id"),
            ...
            # ── P0.2A: Structured error-based retry with correct attempt counts ──
            # retry_count = number of retries ALREADY executed (starts at 0).
            # attempt_number = retry_count + 1 (current attempt just failed).
            # Stop condition: retry_count >= (total_allowed - 1).
            from models.zhipuai.provider_api_error import ProviderAPIError
            from models.zhipuai.exception_classifier import classify

            if isinstance(e, ProviderAPIError):
                perr = e
            elif isinstance(e, AgentCancelledError):
                raise
            else:
                info = classify(e)
                perr = ProviderAPIError.from_classified(str(e)[:200], info)

            # ── Context overflow → independent handling ──
            if "[CONTEXT_OVERFLOW]" in str(e) or (
                perr.is_client_error and perr.status_code == 400
            ):
                error_str_lower = str(e).lower()
                is_overflow = any(kw in error_str_lower for kw in [
                    'context length exceeded', 'context overflow', 'context window',
                    'too large', 'exceeds model context', 'request_too_large', 'tokens exceed',
                    'maximum context length', 'prompt is too long', 'request exceeds',
                ])
                if is_overflow:
                    ...
                    if not _overflow_retry:
                        trimmed = self._aggressive_trim_for_overflow()
                        if trimmed:
                            logger.warning("🔄 Trimmed context, retrying...")
                            return self._call_llm_stream(
                                retry_on_empty=retry_on_empty,
                                retry_count=retry_count, max_retries=max_retries,
                                _overflow_retry=True)
                    self.messages.clear(); self._clear_session_db()
                    raise Exception(_t(
                        "抱歉，对话历史过长导致上下文溢出。我已清空历史记录，请重新描述你的需求。",
                        "Sorry, the conversation history got too long. I've cleared the history."))

            # ── Non-retryable client errors (400/401/403/404) ──
            if perr.is_client_error:
                # GLM 400/1210: one-shot sanitized fallback (independent of overflow)
                error_str_lower = str(e).lower()
                is_1210 = "1210" in error_str_lower
                already_sanitized = getattr(self, '_sanitized_retry_attempted', False)
                if is_1210 and not already_sanitized and not retry_count:
                    self._sanitized_retry_attempted = True

                    # Check if current message has image(s)
                    current_has_image = False
                    current_image_ok = True
                    last_user = None
                            ...
                            for b in content:
                                if isinstance(b, dict) and b.get("type") == "image":
                                    ...
                                    if not isinstance(src, dict) or not src.get("data"):
                                        current_image_ok = False
                                        break

                    if current_has_image and not current_image_ok:
                        # Bad current image → fail closed, don't call LLM blind
                        logger.error("🔄 GLM 400/1210: current image invalid, fail closed")
                        raise Exception(_t(
                            "图片处理失败，请重新发送",
                            "Image processing failed, please resend"
                        ))

                    logger.warning("🔄 GLM 400/1210 detected — attempting sanitized fallback")
                    fallback_msgs = self._sanitized_fallback_messages(
                        self.messages, getattr(self, 'system_prompt', ''),
                        keep_current_images=current_has_image)
                    if fallback_msgs:
                        logger.info(
                            f"🔧 Sanitized fallback: {len(fallback_msgs)} msgs "
                            f"(was {len(self.messages)}) keep_images={current_has_image}")
                        saved = self.messages
                        ...
                        try:
                            ...
                            self.messages = saved
                # 1261 (context overflow) → handled above by overflow keywords
                # Regular 400 (non-1210, non-1261) → no retry
                logger.error(f"❌ Non-retryable client error: {perr}")
                raise

            # ── Connection error: 2 total attempts (initial + 1 retry with rebuild) ──
            if perr.is_connection_error:
                if retry_count >= 1:  # Already retried once → stop
                    logger.error(f"❌ Connection error after 2 attempts: {perr}")
                    raise
                logger.warning(f"⚠️ Connection error (attempt {retry_count+1}/2): {perr}")
                if bot and hasattr(bot, '_reset_client'):
                    ok = bot._reset_client(request_client_gen, "connection_error_retry")
                    logger.info(
                        "[AgentStream] Client rebuild: %s (expected_gen=%d)",
                        "OK" if ok else "already rebuilt by another thread",
                        request_client_gen,
                    )
                time.sleep(2 + random.random() * 1)
                return self._call_llm_stream(
                    retry_on_empty=retry_on_empty,
                    retry_count=retry_count + 1, max_retries=1)

            # ── Timeout: 2 total attempts ──
            if perr.is_timeout:
                if retry_count >= 1:
                    logger.error(f"❌ Timeout after 2 attempts: {perr}")
                    raise
                logger.warning(f"⚠️ Timeout (attempt {retry_count+1}/2): {perr}")
                time.sleep(2 + random.random() * 1)
                return self._call_llm_stream(
                    retry_on_empty=retry_on_empty,
                    retry_count=retry_count + 1, max_retries=1)

            # ── Rate limit (429): 2 total attempts, respect Retry-After ──
            if perr.is_rate_limit:
                if retry_count >= 1:
                    logger.error(f"❌ Rate limit after 2 attempts: {perr}")
                    raise
                wait = perr.retry_after_seconds or (30 + retry_count * 15)
                wait = min(wait, 60)
                logger.warning(f"⚠️ Rate limit (attempt {retry_count+1}/2), waiting {wait}s: {perr}")
                time.sleep(wait)
                return self._call_llm_stream(
                    retry_on_empty=retry_on_empty,
                    retry_count=retry_count + 1, max_retries=1)

            # ── Server error (500/502/503/504): 3 total attempts, exp backoff ──
            if perr.is_server_error:
                if retry_count >= 2:  # Already retried twice → 3 attempts total
                    logger.error(f"❌ Server error after 3 attempts: {perr}")
                    raise
                wait = min((2 ** retry_count) + random.random() * 2, 15)
                logger.warning(f"⚠️ Server error (attempt {retry_count+1}/3), waiting {wait:.1f}s: {perr}")
                time.sleep(wait)
                return self._call_llm_stream(
                    retry_on_empty=retry_on_empty,
                    retry_count=retry_count + 1, max_retries=2)

            # ── Unknown/other retryable errors ──
            if perr.retryable and retry_count < max_retries:
                wait = min((retry_count + 1) * 2 + random.random(), 10)
                logger.warning(f"⚠️ Retryable error (attempt {retry_count+1}/{max_retries+1}), waiting {wait:.1f}s: {perr}")
                time.sleep(wait)
                return self._call_llm_stream(
                    retry_on_empty=retry_on_empty,
            ...
            # ── Default: raise ──
            logger.error(f"❌ LLM call error: {perr}")
            raise
    ...
    def _validate_and_fix_messages(self):
        """Enhanced validation: fix empty content, strip history Base64,
        preserve tool pairs, ensure valid role sequence (system→user...)."""
        msgs = self.messages
        fixes = 0

        # 1. Shared sanitizer (tool-use adjacency, orphans)
        fixes += sanitize_claude_messages(msgs)

        # 2. Strip Base64 from old history images
        #    Keep: current turn + previous turn for follow-up questions
        fixes += self._strip_history_base64()

        # 3. Fix empty or None content (preserves tool_calls on assistant)
        fixes += self._fix_empty_content()

        # 4. Ensure valid role sequence: optional system, then user
        fixes += self._fix_leading_role()

        if fixes:
            logger.info(f"🔧 Message validation: {fixes} fix(es) applied")

    def _strip_history_base64(self) -> int:
        """Strip Base64 from history images, keeping current + previous turn.

        Diagnostic tracks:
          - current_image_count / current_image_chars (current turn)
          - historical_image_count / historical_image_chars (older, stripped)
        Returns count of blocks stripped/replaced.
        """
        stripped = 0
        # Find index of current-turn user message and previous-turn user message
        user_indices = []
        ...
        keep_indices = set()
        if user_indices:
            keep_indices.add(user_indices[-1])  # current turn
        if len(user_indices) >= 2:
            keep_indices.add(user_indices[-2])  # previous turn (follow-up)
    ...
    def _fix_empty_content(self) -> int:
        """Fix messages with None/empty content.

        Rules:
        - assistant with None content BUT valid tool_calls → KEEP (valid)
        - tool messages must have tool_call_id
        - plain user/assistant empty content → fix or drop
        """
        fixed = 0
        i = 0
        while i < len(self.messages):
            ...
            # assistant with None content but has valid tool_calls → keep
            ...
            if content is None:
                if role == "user":
                    msg["content"] = [{"type": "text", "text": "[消息内容缺失]"}]
                    fixed += 1
                elif role == "assistant":
    ...
    def _fix_leading_role(self) -> int:
        """Ensure valid opening sequence: optional system(s), then user.

        Keeps all leading system messages (personality prompt).
        After the last system message, the first conversation message
        MUST be 'user'. Drops orphaned assistant/tool messages
        that appear between system and user.
        """
        dropped = 0
        ...
        # After last system (or from start if no system), drop until user
        start_check = last_system_idx + 1 if last_system_idx >= 0 else 0
        while start_check < len(self.messages):
            role = self.messages[start_check].get("role")
            if role == "user":
                break
            if role in ("assistant", "tool"):
                self.messages.pop(start_check)
                dropped += 1
            else:
                break  # unknown role, stop
        return dropped

    def _diagnose_payload(self, messages, tools_schema) -> dict:
        """Anonymous payload structure diagnostic. Never logs content/keys/IDs.

        Distinguishes current vs historical images for GLM 400/1210 analysis.
        """
        import json
        ...
        diag = {
            ...
            "current_image_chars": 0,
            "historical_image_count": 0,
            "historical_image_chars": 0,
            "tool_use_count": 0,
            "tool_result_count": 0,
            "tool_pair_ok": True,
            "orphaned_tool_use": 0,
            "orphaned_tool_result": 0,
            "max_content_chars": 0,
            "payload_total_chars": 0,
            ...
            if isinstance(content, list):
                for block in content:
                    ...
                    if bt == "image":
                        ...
                        chars = len(data) if data else 0
                        # Current + previous turn → current; older → historical
                        in_keep = (idx == current_user_idx or idx == prev_user_idx)
                        if in_keep:
                            diag["current_image_count"] += 1
                            diag["current_image_chars"] += chars
                        else:
                            diag["historical_image_count"] += 1
                            diag["historical_image_chars"] += chars
                    elif bt == "tool_use":
                        diag["tool_use_count"] += 1
                        tid = block.get("id")
                        if tid:
                            tool_use_ids.add(tid)
                    elif bt == "tool_result":
                        diag["tool_result_count"] += 1
                        tid = block.get("tool_use_id")
                        if tid:
                            tool_result_ids.add(tid)
            try:
                raw = json.dumps(msg, ensure_ascii=False)
                diag["payload_total_chars"] += len(raw)
                diag["max_content_chars"] = max(diag["max_content_chars"], len(raw))
            except Exception:
                pass

        diag["tool_pair_ok"] = tool_use_ids == tool_result_ids
        diag["orphaned_tool_use"] = len(tool_use_ids - tool_result_ids)
        diag["orphaned_tool_result"] = len(tool_result_ids - tool_use_ids)
        # Backward-compat totals
        diag["image_count"] = diag["current_image_count"] + diag["historical_image_count"]
        diag["image_chars"] = diag["current_image_chars"] + diag["historical_image_chars"]
        return diag

    def _sanitized_fallback_messages(self, messages, system_prompt,
                                       keep_current_images: bool = False) -> list:
        """Build safe message list for one-shot 400/1210 retry.

        Structure:
          system message(s)
          + last 5 complete user/assistant pairs
          + current user message (only once)

        If keep_current_images=True: preserves images in the last user message.
        Older images are ALWAYS stripped → text placeholder (don't pretend to have seen them).
        """
        keep_idx = -1
        if keep_current_images:
            for i in range(len(messages) - 1, -1, -1):
                if messages[i].get("role") == "user":
                    keep_idx = i
                    break
        ...
        if not system_prompt:
            # Even without explicit prompt, keep any existing system msgs
            pass

        # Collect user text + following assistant text into pairs
        pairs = []
        current_pair = None
        for msg in messages:
            role = msg.get("role")
            ...
            if role == "user" and text:
                if current_pair and current_pair.get("user"):
                    pairs.append(current_pair)
                current_pair = {"user": text, "assistant": ""}
            elif role == "assistant" and text and current_pair:
                current_pair["assistant"] = text
        if current_pair and current_pair.get("user"):
            pairs.append(current_pair)

        # Keep last 5 pairs, preserve the LAST user as current
        recent = pairs[-5:]
        for p in recent[:-1]:
            clean.append({"role": "user", "content": p["user"]})
            clean.append({"role": "assistant", "content": p.get("assistant", "") or "[历史回复]"})
        if recent:
            last_text = recent[-1]["user"]
            # If keep_current_images and we know the original index, preserve images
            if keep_current_images and keep_idx >= 0 and keep_idx < len(messages):
                orig_content = messages[keep_idx].get("content", [])
                if isinstance(orig_content, list):
                    ...
                    for b in orig_content:
                        if isinstance(b, dict) and b.get("type") == "image":
                            ...
                            has_image = True
                    if has_image:
                        new_blocks.append({"type": "text", "text": last_text})
                        clean.append({"role": "user", "content": new_blocks})
                    else:
                        clean.append({"role": "user", "content": last_text})
                else:
                    clean.append({"role": "user", "content": last_text})
            else:
                clean.append({"role": "user", "content": last_text})
    ...
    def _compact_historical_tool_turns(self) -> int:
        """Replace completed historical tool chains with their final text.

        Raw search/browser results are useful only while the current turn is
        executing.  Keeping them for later turns caused 100K+ character prompts
        and abrupt half-history trimming.  Completed turns retain the user's
        question and the final assistant answer; the newest/current turn stays
        untouched so tool-call pairing remains valid during execution.
        """
        turns = self._identify_complete_turns()
        if len(turns) < 2:
            return 0
        ...
        for index, turn in enumerate(turns):
            is_current_turn = index == len(turns) - 1
            has_tools = any(
                isinstance(block, dict)
            ...
            if has_tools and not is_current_turn:
                compacted = compress_turn_to_text_only(turn)
                compacted_turns.append(
                    compacted if compacted.get("messages") else turn
                )
                changed += 1
            else:
        ...
        if changed:
            ...
            logger.info(
                "📦 Compacted %d completed historical tool turn(s) to text-only",
                changed,
            )
        return changed
    ...
    def _trim_messages(self):
        ...
        # Step 0: completed tool turns no longer need raw intermediate payloads.
        self._compact_historical_tool_turns()

        # Step 0b: truncate any remaining historical tool result (defensive).
